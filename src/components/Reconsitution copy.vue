<template>
  <div class="PlanTabs fixedHeight">
    ...
    <!-- <div class="forecast" v-show="activeName=='forecast'">
            <div class="frequency">
                <el-button v-for="index in 6" :key="index" @click="canShow(index)">信号{{ index }}</el-button>
            </div>
            <div class="myTitle">
                <p>信道占用</p>
            </div>
            <div class="topGroup" ref="topGroup">
                ...
                <div class="scrollView">
                    <ul ref="occupy">
                        <li v-for="(item,index) in actualValue" :key="index" v-show="(index+1)%point==0||point=='-1'">
                            <p :class="setShow(item,index,'blue')"></p>
                        </li>
                    </ul>
                    <ul ref="idle" class="specialRed">
                        <li v-for="(item,index) in predictValue" :key="index" v-show="(index+1)%point==0||point=='-1'">
                            <p :class="setShow(item,index,'blue')"></p>
                        </li>
                    </ul>
                </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      //实际值
      actual: (state) => state.data["actual_status"],
      //预测值
      predict: (state) => state.data["predict_status"],
    }),
  },
  watch: {
    actual() {
  ...
  methods: {
    ...
    // },
    Rendering() {
      this.actualValue.push(...this.actual);
      this.predictValue.push(...this.predict);
    },
    canShow(index) {
      this.point = index;
    },
